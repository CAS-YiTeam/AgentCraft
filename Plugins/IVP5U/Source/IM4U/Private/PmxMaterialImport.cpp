...
)
{
	...
	if (UnrealMaterial->GetEditorOnlyData()->AmbientOcclusion.Expression == NULL)
	{
		...
		TArray<FExpressionOutput> Outputs = UnrealMaterial->GetEditorOnlyData()->AmbientOcclusion.Expression->
			GetOutputs();
		FExpressionOutput* Output = Outputs.GetData();
...
if (UnrealMaterial->AmbientOcclusion.Expression == NULL)
{
	...
	TArray<FExpressionOutput> Outputs = UnrealMaterial->AmbientOcclusion.Expression->
		GetOutputs();
	FExpressionOutput* Output = Outputs.GetData();
...
)
{
	...
	{
		/*
		 * 以下，从附带IM4U插件的基本材质进行复制，制作MaterialInstance的模式
		 */
	...
	TArray<UTexture*>& textureAssetList)
{
...
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION > 0
			StaticParams.GetRuntime().StaticSwitchParameters.Add(Param);
#else
	...
	}
	{
		...
		if (nullptr != ColorTex)
		{
			...
			Param.bOverride = true;
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION > 0
			StaticParams.GetRuntime().StaticSwitchParameters.Add(Param); //DestSeq->MarkRawDataAsModified();
#else
			StaticParams.StaticSwitchParameters.Add(Param);
#endif
	...
	// StaticSwitch
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION > 0
	if (0 < StaticParams.GetRuntime().StaticSwitchParameters.Num())
	{
...
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION > 0
		StaticParams.GetRuntime().StaticSwitchParameters.Add(Param);//5.2失效
#else
